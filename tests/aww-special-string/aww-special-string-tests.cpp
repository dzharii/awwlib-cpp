...
TEST_CASE("safe_filename_from_string handles typical and edge cases") {
  SUBCASE("Empty input yields empty output") {
    CHECK(safe_filename_from_string("").empty());
  }

  SUBCASE("Already safe name stays unchanged") {
    CHECK_EQ(safe_filename_from_string("report_2025.txt"), "report_2025.txt");
  }
  ...
  SUBCASE("Consecutive unsafe runs fold into one underscore") {
    CHECK_EQ(safe_filename_from_string("file///:??name"), "file_name");
  }

  SUBCASE("Trailing dots are removed") {
    CHECK_EQ(safe_filename_from_string("logfile."), "logfile");
  }
  ...
  SUBCASE("Control characters are treated as unsafe") {
    ...
    CHECK_EQ(safe_filename_from_string(with_ctrl), "file_name");
  }

  SUBCASE("Russian input: colon replaced with underscore") {
    CHECK_EQ(safe_filename_from_string(u8sv(u8"пример:файл.txt")), u8s(u8"пример_файл.txt"));
  }

  SUBCASE("Japanese input: slash replaced with underscore") {
    CHECK_EQ(safe_filename_from_string(u8sv(u8"データ/バックアップ")), u8s(u8"データ_バックアップ"));
  }
